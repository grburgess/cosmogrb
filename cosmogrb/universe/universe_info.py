...
class UniverseInfo(object):
    ...
    def write(self, file_name):
        ...
        with h5py.File(file_name, "w") as f:
            ...
            grbs = h5py.create_dataset(
                "grb_saves", data=np.array(self._grb_save_files, dtype="s100")
            )

            if self._is_processed:

                grb_dets = h5py.create_dataset(
                    "grb_dets", data=np.array(self._grb_detector_files, dtype="s100")
                )
