...
cosmogrb_dev_log_handler = handlers.TimedRotatingFileHandler(
...
cosmogrb_dev_log_handler.setFormatter(_dev_formatter)
cosmogrb_dev_log_handler.setLevel(logging.DEBUG)
# now set up the usr log which will save the info

cosmogrb_usr_log_handler = handlers.TimedRotatingFileHandler(
    get_path_of_log_file("usr.log"), when="D", interval=1, backupCount=10
)

cosmogrb_usr_log_handler.setLevel(logging.INFO)
...
cosmogrb_usr_log_handler.setFormatter(_usr_formatter)
...
cosmogrb_console_log_handler.setLevel("INFO")
...
def silence_warnings():
    ...
    cosmogrb_usr_log_handler.addFilter(warning_filter)
    cosmogrb_console_log_handler.addFilter(warning_filter)
...
def activate_warnings():
    ...
    cosmogrb_usr_log_handler.removeFilter(warning_filter)
    cosmogrb_console_log_handler.removeFilter(warning_filter)


def update_logging_level(level):

    cosmogrb_console_log_handler.setLevel(level)
...
def setup_logger(name):
    ...
    log.addHandler(cosmogrb_dev_log_handler)

    log.addHandler(cosmogrb_console_log_handler)

    log.addHandler(cosmogrb_usr_log_handler)
